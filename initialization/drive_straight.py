...
def cmd_align(args) -> None:
    ...
    Why this exists: a leg driven out on dead reckoning missed its start point by 0.373 m
    and 9.2 deg (2026-09-04), which does not fit a car door. The exit has to be referenced
    to the doorway itself, and the lidar is the only sensor that can see it — the chest
    camera faces the doors only from inside, and looks the wrong way entirely before the
    robot has entered. The measurement is cheap enough for dead time: the scan is 2.00 Hz
    with 74 ms of latency, so two confirming reads cost about a second against a 60 s
    press. Take it while the car is moving and leave only the straight leg for the door's
    open window.
    ...
    try:
        ...
        if straight >= half and slack_straight > resid:
            ...
            print(f"  VERDICT: TIGHT — the clear band is only +-{room:.0f} deg and the "
                  f"base cannot turn finer than {TURN_FLOOR_DEG:.0f} deg")
    finally:
        ch.close()
...
def main() -> int:
    ...
    al.add_argument("metres", type=float,
                    help="the leg you intend to drive, signed like `move`")
    al.add_argument("--window", type=float, default=1.20, metavar="M",
                    help="ignore returns further than this to either side")
    al.add_argument("--sweep", type=float, default=40.0, metavar="DEG",
                    help="heading range to test either side of straight ahead")
    al.add_argument("--margin", type=float, default=0.05, metavar="M",
                    help="clearance wanted beyond the robot's own half-width, per side")
